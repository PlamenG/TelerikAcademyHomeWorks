// Solution from - https://github.com/jasssonpet/TelerikAcademy/blob/master/Programming/2.CSharpPartTwo/4.NumeralSystems/9.ShowFloatingPoint/Program.cs
...
class Program
{
    ...
    static string GetExponent(string integer, string fraction)
    {
        ...
        if (integer.Length != 0)
        {
            exponent = integer.Length - 1; // 8 = 2 ^ 3 in binary is 1000 - power is +3 - positive
        }
        else
        {
            exponent = -fraction.IndexOf('1') - 1; // 0.125 = 1 / 8 in binary is 0.001 - power is -3 - negative
        }
    ...
    static string GetMantissa(string integer, string fraction)
    {
        ...
        if (integer.Length != 0)
        {
            mantissa = integer.Substring(1) + fraction; // First is always 1 (if normalized)
        }
        else
        {
            mantissa = fraction.Substring(fraction.IndexOf('1') + 1); // No integer part - get first non-zero in fraction
        }
